...
Future<void> exportStatementToCSV(StatementExportData data) async {
  ...
  String csvData = const ListToCsvConverter().convert(rows);
  final directory = await getApplicationDocumentsDirectory();
  final path = '${directory.path}/split_pro_statement_${data.scope}.csv';
  final file = File(path);
  await file.writeAsString(csvData);
  await Share.shareXFiles([XFile(path)], text: 'Split Pro Statement (CSV)');
}
...
Future<void> exportStatementToPDF(StatementExportData data) async {
  ...
  final directory = await getApplicationDocumentsDirectory();
  final path = '${directory.path}/split_pro_statement_${data.scope}.pdf';
  final file = File(path);
  await file.writeAsBytes(pdfBytes);
  await Share.shareXFiles([XFile(path)], text: 'Split Pro Statement (PDF)');
}
