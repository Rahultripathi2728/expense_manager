...
class _PaymentSummaryPageState extends ConsumerState<PaymentSummaryPage> {
  ...
  Future<void> _settleSingleTransaction({
    required SimplifiedTransaction tx,
    required Map<String, Profile> profiles,
    required List<Expense> unsettledExpenses,
  }) async {
    ...
    try {
      ...
      final expenseList = (widget.selectedExpenses != null && widget.selectedExpenses!.isNotEmpty)
          ? widget.selectedExpenses!
          : unsettledExpenses;
      ...
      final recipientName = profiles[tx.toUserId]?.fullName ?? 'Member';
      ...
      _settledTransactionKeys.remove(key);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _settleAllMyPayments({
    required List<SimplifiedTransaction> myTransactions,
    required Map<String, Profile> profiles,
    required List<Expense> unsettledExpenses,
  }) async {
    if (_isSettling) return;

    final txToProcess = <SimplifiedTransaction>[];
    final newlyAddedKeys = <String>[];
    for (final tx in myTransactions) {
      final key = '${tx.fromUserId}_${tx.toUserId}_${tx.amount}';
    ...
    if (txToProcess.isEmpty) return;

    setState(() {
      _isSettling = true;
      _settledTransactionKeys.addAll(newlyAddedKeys);
    });
    ...
    try {
      ...
      _settledTransactionKeys.removeAll(newlyAddedKeys);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    return Scaffold(
      ...
      body: balanceAsync.when(
        ...
        data: (data) {
          ...
          final myTransactions = allTransactions.where((t) => t.fromUserId == myUserId).toList();

          final pendingMyTransactions = myTransactions.where((t) {
          ...
          final otherTransactions = allTransactions.where((t) => t.fromUserId != myUserId).toList();
                ...
                children: [
                  ...
                  if (myTransactions.isNotEmpty) ...[
                    ...
                    ...myTransactions.map((tx) {
                      final toProfile = profiles[tx.toUserId];
                  ...
                  if (otherTransactions.isNotEmpty) ...[
                    ...
                    ...otherTransactions.map((tx) {
                      final fromName = profiles[tx.fromUserId]?.fullName ?? 'Member';
                      final toName = profiles[tx.toUserId]?.fullName ?? 'Member';
      ...
      bottomNavigationBar: balanceAsync.maybeWhen(
        data: (data) {
          final allTransactions = data.transactions;
          final myTransactions = allTransactions.where((t) => t.fromUserId == myUserId).toList();
          final pendingMyTransactions = myTransactions.where((t) {
          ...
          if (pendingMyTransactions.isEmpty) return null;
